...
function main() {
  ...
  // Set clear color to a dark blue (to give me solar system vibes)
...
function drawScene(deltaTime) {
  ...
  const numPlanets = 10;
  const spacingMultiplier = 3.25;	//Makes it easier to space out planets from Sun
  const planetSizes = [0.5, 0.7, 0.8, 0.9, 1.1, 1.3, 1.5, 1.8, 2.2, 2.5];
  const planetDistances = [0, 0.25, 0.625, 1, 1.25, 2, 2.25, 3, 3.25, 4];
  const planetRotations = [0.22, 0.2, 0.18, 0.16, 0.14, 0.12, 0.1, 0.08, 0.06, 0.04];
  const sunSize = 2.7; 
  
  for(let i = 0; i < numPlanets; i++){
	  let modelMatrix = glMatrix.mat4.create();
	  let planetSize = planetSizes[i];
	  let planetDistance = planetDistances[i] * spacingMultiplier;
	  let planetRotationSpeed = planetRotations[i];
	  let sunWorldPos = [0.0, 0.0, 0.0];
	  let planetWorldPos = [0.0, planetDistance, -6.0];
	  let rotationAxis = [1.0, 0.0, 0.0];

      //If it's not the sun, rotate around the sun
	  if(i > 0){
		  glMatrix.mat4.rotate(modelMatrix, modelMatrix, globalTime * planetRotationSpeed, rotationAxis);
		  glMatrix.mat4.translate(modelMatrix, modelMatrix, planetWorldPos);
      //If it is the sun, rotate in place
	  }else{
		  glMatrix.mat4.translate(modelMatrix, modelMatrix, sunWorldPos);
		  glMatrix.mat4.rotate(modelMatrix, modelMatrix, globalTime, [1.0, 1.0, 1.0]);
		  planetSize = sunSize;
	  }

	  //Scale the planets (again not to scale)
	  let scaleMatrix = glMatrix.mat4.create();
      glMatrix.mat4.scale(scaleMatrix, scaleMatrix, [planetSize, planetSize, planetSize]);
