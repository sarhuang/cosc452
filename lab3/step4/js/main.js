...
async function setupScene() {	
	let objData = await loadNetworkResourceAsText('../../shared/resources/models/sphereTextured.obj');
	let vertSource = await loadNetworkResourceAsText('../../shared/resources/shaders/verts/enviromap300.vert');
	let fragSource = await loadNetworkResourceAsText('../../shared/resources/shaders/frags/enviromap300.frag');
	...
	const imageFaces = [
	  {
		...
		url: '../../shared/resources/enviromaps/coit_tower/posx.jpg',
	  },
	  { 
		face: gl.TEXTURE_CUBE_MAP_NEGATIVE_X,
		url: '../../shared/resources/enviromaps/coit_tower/negx.jpg',
	  },
	  {
		face: gl.TEXTURE_CUBE_MAP_POSITIVE_Y,
		url: '../../shared/resources/enviromaps/coit_tower/posy.jpg',
	  },
	  {
		face: gl.TEXTURE_CUBE_MAP_NEGATIVE_Y,
		url: '../../shared/resources/enviromaps/coit_tower/negy.jpg',
	  },
	  {
		face: gl.TEXTURE_CUBE_MAP_POSITIVE_Z,
		url: '../../shared/resources/enviromaps/coit_tower/posz.jpg',
	  },
	  {
		face: gl.TEXTURE_CUBE_MAP_NEGATIVE_Z,
		url: '../../shared/resources/enviromaps/coit_tower/negz.jpg',
	  },
	];

	imageFaces.forEach((img) => {
		const {face, url} = img;
		gl.texImage2D(face, 0, gl.RGBA, 2048, 2048, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
		image = new Image();
		image.src = url;
		console.log(image);
...
function initializeMyObject(vertSource, fragSource, objData) {
	...
	myDrawable.uniformLocations = myShader.getUniformLocations(['uModelViewMatrix', 'uProjectionMatrix', 'uNormalMatrix', 'uSampler',
																'uCameraPosition']);
